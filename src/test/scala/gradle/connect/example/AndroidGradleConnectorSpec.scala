...
class AndroidGradleConnectorSpec extends FlatSpec with BeforeAndAfter {
  ...
  val APP_BUILD_GENERATED_DIR = s"$APP_BUILD_DIR/${AndroidProject.FD_GENERATED}"
  ...
  "android project model" should "have flavor dimensions" in {
    assert(android.getFlavorDimensions.isEmpty)
  }

  "android project model" should "have framework sources" in {
    assert(android.getFrameworkSources.isEmpty)
  }

  "android project model" should "have java compiler options" in {
  ...
  "android { lintOptions {...} }" should "have some elements" in {
  ...
  "android project model" should "have builder model version" in {
    assert(android.getModelVersion == "2.1.2")

  }

  "android project model" should "have name" in {
    assert(android.getName == "app")
  }

  "android project model" should "have native tool chains " in {
    assert(android.getNativeToolchains.isEmpty)
  }

  "android project model" should "have plugin generation" in {
    assert(android.getPluginGeneration == 1)
  }

  "android {...}" should "have resource prefix" in {
    assert(android.getResourcePrefix == null)
  }

  "android project model" should "have sync issues" in {
    assert(android.getSyncIssues.isEmpty)
  }

  "android project" should "have two variants" in {
    assert(android.getVariants.size() == 2)
  }

  "android project" should "have develop-debug variant" in {
    val developDebug = android.getVariants.asScala.head
    ...
    developDebug.getExtraJavaArtifacts.asScala match {
      case extraJavaArtifacts =>
        ...
        assert(artifact.getName == AndroidProject.ARTIFACT_UNIT_TEST)
    ...
    developDebug.getMainArtifact match {
      case mainArtifact =>
        ...
        assert(mainArtifact.getName == AndroidProject.ARTIFACT_MAIN)
  ...
  "android project" should "have develop-release variant" in {
    val developRelease = android.getVariants.asScala.last
    ...
    developRelease.getExtraJavaArtifacts.asScala match {
      case extraJavaArtifacts =>
        ...
        assert(extraJavaArtifacts.head.getName == AndroidProject.ARTIFACT_UNIT_TEST)
  ...
  "android project model" should "have library flag" in {
    assert(!android.isLibrary)
  }

  "android{ buildTypes {...} }" should "have two elements" in {
    assert(android.getBuildTypes.size() == 2)
  }

  "android{ buildTypes { debug {...} }" should "have some elements" in {
    val buildTypes = android.getBuildTypes.asScala.toList
    val debug = buildTypes.head
    ...
    debug.getExtraSourceProviders.asScala match {
      case extraSourceProviders =>
        ...
        assert(extraSourceProvider.getArtifactName == AndroidProject.ARTIFACT_UNIT_TEST)
  ...
  "android{ buildTypes { release {...} }" should "have some elements" in {
    val buildTypes = android.getBuildTypes.asScala.toList
    val release = buildTypes.last
    ...
    release.getSourceProvider match {
      case sourceProvider =>
        assert(sourceProvider.getAidlDirectories.size() == 1)
        assert(sourceProvider.getAidlDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/aidl"))
        assert(sourceProvider.getAssetsDirectories.size() == 1)
        assert(sourceProvider.getAssetsDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/assets"))
        assert(sourceProvider.getCDirectories.size() == 1)
        assert(sourceProvider.getCDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/jni"))
        assert(sourceProvider.getCppDirectories.size() == 1)
        assert(sourceProvider.getCppDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/jni"))
        assert(sourceProvider.getJavaDirectories.size() == 1)
        assert(sourceProvider.getJavaDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/java"))
        assert(sourceProvider.getJniLibsDirectories.size() == 1)
        assert(sourceProvider.getJniLibsDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/jniLibs"))
        assert(sourceProvider.getManifestFile.getAbsolutePath.contains(s"$APP_SRC_DIR/release/AndroidManifest.xml"))
        assert(sourceProvider.getName == "release")
        assert(sourceProvider.getRenderscriptDirectories.size() == 1)
        assert(sourceProvider.getRenderscriptDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/rs"))
        assert(sourceProvider.getResDirectories.size() == 1)
        assert(sourceProvider.getResDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/res"))
        assert(sourceProvider.getResourcesDirectories.size() == 1)
        assert(sourceProvider.getResourcesDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/resources"))
        assert(sourceProvider.getShadersDirectories.size() == 1)
        assert(sourceProvider.getShadersDirectories.asScala.head.getAbsolutePath.contains(s"$APP_SRC_DIR/release/shaders"))
    }
  }

  "android { defaultConfig {...} }" should "have some elements" in {
  ...
  "android { productFlavors {...} }" should "have some elements" in {
      ...
      case productFlavors =>
        ...
        productFlavor.getExtraSourceProviders.asScala match {
          case extraSourceProviders =>
            ...
            extraSourceProviders.head match {
              case androidTest =>
                assert(androidTest.getArtifactName == AndroidProject.ARTIFACT_ANDROID_TEST)
                androidTest.getSourceProvider match {
                  case sourceProvider =>
                    assert(sourceProvider.getAidlDirectories.size() == 1)
            ...
            extraSourceProviders.last match {
              case unitTest =>
                assert(unitTest.getArtifactName == AndroidProject.ARTIFACT_UNIT_TEST)
  ...
  "android { signingConfigs {...} }" should "have some elements" in {
